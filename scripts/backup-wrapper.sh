...
cleanup_old_images() {
    ...
    while :; do
        ...
        echo "[INFO] Not enough space ($avail < $needed_bytes), deleting oldest: $oldest_week_dir" >&2
...
main() {
    ...
    case "$mode" in
        --initial)
            [[ $# -lt 3 ]] && { echo "[ERROR] Missing args for --initial." >&2; exit 1; }
            mount_point="$1"; backup_script="$2"; image_path="$3"
            ;;
        --incremental)
            [[ $# -lt 3 ]] && { echo "[ERROR] Missing args for --incremental." >&2; exit 1; }
            mount_point="$1"; backup_script="$2"; image_path="$3"
            ;;
        *)
    ...
    if ! [[ "$mount_point" =~ ^/mnt/backup(/|$) ]]; then
        echo "[ERROR] mount-point '$mount_point' must be under /mnt/backup." >&2
        exit 1
    fi
    ...
    if [[ "$mode" == "--initial" ]]; then
        ...
        echo "[INFO] Running: bash \"$backup_script\" -i ${OPTIONS:+-o \"${OPTIONS[*]}\"} \"$args\"" >&2

        if (( ${#OPTIONS[@]} > 0 )); then
            bash "$backup_script" -i -o "$(printf "%s," "${OPTIONS[@]}" | sed 's/,$//')" "$args"
        else
            bash "$backup_script" -i "$args"
        fi

    else
        echo "[INFO] Running: bash \"$backup_script\" ${OPTIONS:+-o \"${OPTIONS[*]}\"} \"$image_path\"" >&2

        if (( ${#OPTIONS[@]} > 0 )); then
            bash "$backup_script" -o "$(printf "%s," "${OPTIONS[@]}" | sed 's/,$//')" "$image_path"
        else
